...
export default function Item({ data, parentId, index }: Props) {
  ...
  const className = cl(
    'p-2 border border-gray-300 border-dashed relative',
    data.canvasClassName,
    {
      'canvas-field': !data.layout,
      'canvas-field-warpper flex-1': parentId === 'root',
  ...
  return (
    <div onClick={handleFocus} className={className} ref={ref}>
      {state.focusId === data.id && parentId !== 'root' && action}
